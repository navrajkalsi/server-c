...
const char *STATIC_FILES[STATIC_LEN] = {ICON_ICO, SERVER_HTML, SERVER_JS, ERROR_HTML};

// Arrays of absolute paths of each file
// HAVE TO MAINTAIN BOTH arrays in the future
// found out macro concatention only works with string literals and not even char *s
const Str STATIC_PATHS[STATIC_LEN] = {STR(STATIC_PATH(ICON_ICO)), STR(STATIC_PATH(SERVER_HTML)),
...
bool handle_request(Client *client)
{
  ...
  if (!(client->request_method = c.head).data)
  { // Request is not a valid http request
    client->response_status = ASSIGN_IF_NULL(client->response_status, "400 Bad Request");
    return err("Cutting method", false); // Cannot do much else in this func now
  }
  if (!validate_method(&client->request_method))
  { // Method is invalid
    client->response_status = ASSIGN_IF_NULL(client->response_status, "405 Method Not Allowed");
  ...
  if (!(client->request_path = path_cut.head).data)
  { // Request is not a valid http request
    client->response_status = ASSIGN_IF_NULL(client->response_status, "400 Bad Request");
  ...
  if (!validate_path(&client->request_path, &client->request_static))
  {                      // Path is not valid for some reason
    if (errno == EINVAL) // path is invalid, does not start with / or failed to decode or simplify
      client->response_status = ASSIGN_IF_NULL(client->response_status, "400 Bad Request");
    else if (errno == EACCES)
      client->response_status = ASSIGN_IF_NULL(client->response_status, "403 Forbidden");
    else if (errno == ENOENT) // path does not exist
      client->response_status = ASSIGN_IF_NULL(client->response_status, "404 Not Found");
  ...
  if (equals(&c.head, &STR("HTTP/2.0")) || equals(&c.head, &STR("HTTP/3.0")))
    client->response_status =
        ASSIGN_IF_NULL(client->response_status, "505 HTTP Version Not Supported");
  else
  {
...
bool validate_method(const Str *method)
{
  ...
  return equals(method, &(STR("GET"))) && print_debug("Request method validated");
}
...
bool decode_path(Str *path)
{
  ...
  while (pos < path->len)
  {
    if (pos < path->len - 2 && path->data[pos] == '%')
    { // decoding %
      char *end = NULL;
...
bool set_connection(Str *connection, Str *request)
{
  ...
  if (strncasecmp(c.head.data, "keep-alive", sizeof("keep-alive") - 1) == 0)
    ...
    *connection = STR("close");
    ...
    return false; // now the connection has to be set depending on the http version
...
void print_request(const Client *client)
{
  ...
  inet_ntop(AF_INET6, &(((struct sockaddr_in6 *)&client->address)->sin6_addr), ipstr, sizeof ipstr);

  // printf("%s: %.*s", ipstr, (int)client->request.len,
  // client->request.data);
  printf("\n(%s) %.*s %.*s\n", ipstr, (int)client->request_method.len, client->request_method.data,
         (int)client->request_path.len, client->request_path.data);
}

// returns -1 on error, 0 if not static, 1 if static
// ABSOLUTE path is set, this is the only place in the server where absolute paths are used
